...
class DeepLearner():
    ACTIONS_COUNT = 5  # number of valid actions. In this case up, down, left, right, #fire#, none
    ...
    def __init__(self, scope, checkpoint_path="deep_q_spaceshooter_networks", playback_mode=False, verbose_logging=False):
        ...
        self._input_layer, self._output_layer = self._create_network(scope)
    ...
    def get_keys_pressed(self, screen_array, reward, terminal):
        ...
        screen_resized_binary = screen_resized_grayscaled
        ...
        if not self._playback_mode:
            ...
            print("Time: %s random_action_prob: %s reward %s scores differential %s observations %s" % (self._time, self._probability_of_random_action, reward, sum(self._last_scores) / self.STORE_SCORES_LEN, len(self._observations)))
    ...
    def _train(self):
        ...
        if self._time % self.SAVE_EVERY_X_STEPS == 0:
            save_path = self._saver.save(self._session, self._checkpoint_path + '/network', global_step=self._time)
